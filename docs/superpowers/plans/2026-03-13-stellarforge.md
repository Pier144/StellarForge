      ...
      {CATEGORY_GROUPS.map(group => (
        <div key={group.key}>
          ...
          >
            ...
            <span className="text-[10px]">{expandedGroups.has(group.key) ? '▾' : '▸'}</span>
          </button>
          <AnimatePresence>
...
export function useCodeSync() {
  ...
  useEffect(() => {
    ...
    // The implementer should check if window.stellarforge.paradox?.serialize exists and call it,
    // falling back to JSON.stringify if unavailable.
